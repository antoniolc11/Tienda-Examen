...
<body>
    ...
    if (isset($login, $password, $password_repeat)) {
        ...
        if (strlen($password) < 8) {
            $error['lon_error'] = 'La contraseña tiene que tener 8 caracteres.';
        }

        if (!preg_match('`[a-z]`',$password)){
            $error['min_error'] = 'La contraseña debe incluir al menos una minuscula.';
         }

         if (!preg_match('`[A-Z]`',$clave)){
            $error['may_error'] = "La clave debe tener al menos una letra mayúscula";
         }

         if (!preg_match('`[0-9]`',$clave)){
            $error['may_error'] ="La clave debe tener al menos un caracter numérico";
         }
