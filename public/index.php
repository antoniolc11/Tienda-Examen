            ...
            <main class="flex-1 grid grid-cols-3 gap-4 justify-center justify-items-center">
                <?php foreach ($sent as $fila) : ?>
                    <div class="p-6 max-w-xs min-w-full bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <p class="mb-3 font-normal text-gray-700 dark:text-gray-400"> EN STOK: <?= hh($fila['unidades']) ?></p>
                        <?php if ($fila['unidades'] <= 0) { ?>

                            <button disabled class="inline-flex items-center py-2 px-3.5 text-sm font-medium text-center text-white bg-gray-700 rounded-lg dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"> 
                                SIN EXISTENCIAS</button>
