...
class Usuario extends Modelo
{
    ...
    /* Comprueba la validación del usuario (FALTA TERMINAR) */
    public static function validado($login, ?PDO $pdo = null)
    {
        $pdo = $pdo ?? conectar();

        $sent = $pdo->prepare('SELECT *
                                 FROM usuarios JOIN validados ON "usuarios.id = validados.id"
                                WHERE usuario = :login');
        $sent->execute([':login' => $login]);
        $fila = $sent->fetch(PDO::FETCH_ASSOC);
